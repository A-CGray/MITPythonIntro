...
def sort(L, lt = lambda x,y: x < y):
    ...
    The second argument is some function which returns a true or false,
    entries in the list will be sorted by "truest" to "falsest". Here the
    default has been set as less than but inputting a greater than function
    as the second argument would overwrite this and sort the list in descending order instead"""
    if len(L) < 2:
        return L[:]
    else:
        middle = int(len(L)/2)
